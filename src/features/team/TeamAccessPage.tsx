...
export function TeamAccessPage(_props: TeamAccessPageProps) {
  ...
  return <main className="team-access" aria-labelledby="team-title">
    ...
    <div className="team-layout">
      <section className="team-panel"><header><div><h2>登录账号</h2><p>{accounts.filter((item) => item.active).length} 个启用账号</p></div><div className="role-counts">{(['manager','sales','designer'] as TeamRole[]).map((role) => <span key={role}>{ROLE_LABELS[role]} {roleCounts[role]}</span>)}</div></header>
